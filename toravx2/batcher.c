...
#define compare_and_swap(x,i,j) \
  c = _mm256_max_epu16(x[i],_5q); \
  c = _mm256_cmpeq_epi16(c, x[i]); \
  t = _mm256_xor_si256(x[i],x[j]); \
  t = _mm256_and_si256(t,c); \
  x[i] = _mm256_xor_si256(x[i],t); \
  x[j] = _mm256_xor_si256(x[j],t);



void batcher16x84(__m256i *x)
{    
  __m256i _5q, t, c;
  _5q = _mm256_set1_epi16(61445);
